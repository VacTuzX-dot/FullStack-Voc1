    ...
    paths: {
      ...
      "/api/auth/login": {
        post: {
          tags: ["Authentication"],
          summary: t.endpoints.login.summary,
          description: t.endpoints.login.desc + " (Frontend endpoint)",
          requestBody: {
            required: true,
            content: {
              "application/json": {
      ...
      "/api/auth/register": {
        post: {
          ...
          summary:
            lang === "th"
              ? "📝 สมัครสมาชิก (Frontend)"
              : "📝 Register (Frontend)",
          description:
            lang === "th"
              ? "สมัครสมาชิกผ่าน Frontend - ไม่ต้องยืนยันตัวตน"
              : "Register new user from Frontend signup form - No authentication required",
          requestBody: {
            required: true,
            content: {
              "application/json": {
